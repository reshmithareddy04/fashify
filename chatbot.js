document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleChat = async (message) => {
    ...
    if (text.includes("hello") || text.includes("hi")) {
      ...
      addBotMessage(
        "Rainy days, stylish ways! ☔ A Burberry trench in olive or navy, paired with Hunter boots, keeps you dry and dashing. Here are some tips:\n" +
      ...
      addBotMessage(
        "Embrace the chill in style! 🧥 Layer a Max Mara wool coat over a cashmere turtleneck and tailored trousers. Here are some tips:\n" +
        tips.map(tip => `- ${tip}`).join("\n") +
      ...
      addBotMessage(
        "Beat the heat with elegance! ☀️ A linen Zimmermann dress in mint or ivory, paired with Chloé sandals, screams summer 2025. Here are some tips:\n" +
        tips.map(tip => `- ${tip}`).join("\n") +
      ...
      addBotMessage(
        "Mild weather, chic style! 🌤️ A trench coat with a lightweight sweater and jeans is perfect. Here are some tips:\n" +
        tips.map(tip => `- ${tip}`).join("\n") +
